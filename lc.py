    ...
    with open(filename) as fp:
        for line in fp:
            if line.find('#')!=0:
                ...
                a = [f for f in line.strip().split()]
                a[0]=a[0][1:]
                metadata.append(a)
    dict_of_facets = dict(zip(metadata[0], metadata[1]))
...
class LightCurveDB:
    ...
    def __init__(self):
        ...
        self._seq_index=defaultdict(list)
    ...
    def index(self):
        for f in self._collection:
            ...
            self._seq_index[seq].append(self._collection[f])
